...
public struct StatBlock
{
    ...
    [Header("방어")]
    public float defense;     // 방어율 - 들어오는 피해를 % 감소
    public int dodge;                           // 회피 (명중 판정에서 차감)
    public float deathBlowResist; // 죽음의 일격 저항 - 빈사에서 생존확률
    ...
    public StatBlock Scale(int factor)
    {
        StatBlock result;
        result.maxHp = maxHp * factor;
        result.minDamage = minDamage * factor;
        result.maxDamage = maxDamage * factor;
        result.accuracyMod = accuracyMod * factor;
        result.critChance = critChance * factor;
        result.defense = defense * factor;
        result.dodge = dodge * factor;
        result.deathBlowResist = deathBlowResist * factor;
        result.speed = speed * factor;
        result.moveRange = moveRange * factor;
        result.resistance.stun = resistance.stun * factor;
        result.resistance.move = resistance.move * factor;
        result.resistance.poison = resistance.poison * factor;
        result.resistance.disease = resistance.disease * factor;
        result.resistance.bleed = resistance.bleed * factor;
        result.resistance.debuff = resistance.debuff * factor;
        result.resistance.trap = resistance.trap * factor;
        result.damageMultiplier = damageMultiplier * factor;
        result.eblaMultiplier = eblaMultiplier * factor;
        return result;
    }
}
